...
public class City : MonoBehaviour {
	...
	void Update () {
		for(int i = 0; i < width; i++){
			...
			float sine = Mathf.Sin(Time.time  + (float)(j)/1000f + i);


			children[k].position = pos[k];
//			children[k].position += (transform.forward  * sine/10f);


			//move points along x axis
			float perlin = Mathf.PerlinNoise((float) k / 10f + Time.time, (float) k / 10f + Time.time);
//			children[k].position += transform.up  * Mathf.Clamp(Mathf.Round(Mathf.Pow(perlin, 5f)* 10f)/2f, 0.5f, 1f);
